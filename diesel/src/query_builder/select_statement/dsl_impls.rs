...
for SelectStatement<
    FromClause<F>,
    DefaultSelectClause<FromClause<F>>,
    D,
    W,
    O,
    LOf,
...
impl<F, S, D, W, O, LOf, G, H, LC> QueryDsl
for SelectStatement<F, S, D, W, O, LOf, G, H, LC> {}
...
where
    Tab: Table,
    Self: Query,
    <Tab::AllColumns as ValidGrouping<
        (),
...
impl<F, S, D, W, O, LOf, G, H, LC> CombineDsl
for SelectStatement<F, S, D, W, O, LOf, G, H, LC>
where
    Self: Query,
{
